...
struct FFTDataGenerator
{
    ...
    void produceFFTDataForRendering(const juce::AudioBuffer<float>& audioData, const float negativeInfinity)
    {
        ...
        fowardFFT->performFrequencyOnlyForwardTransform(fftData.data());  // [2]
    ...
    {
        //when you change order, recreate the window, fowardFFT, fifo, fftData
        // also reset the fifoIndex
        //things that neeed recreating should be created on the heap via std::make_unique<>
        
        order = newOrder;
        auto fftSize = getFFTSize();
        
        fowardFFT = std::make_unique<juce::dsp::FFT>(order);
...
private:
    FFTOrder order;
    BlockType fftData;
    std::unique_ptr<juce::dsp::FFT> fowardFFT;
